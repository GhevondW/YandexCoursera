...
inline std::string GetCommentByValue(int value)
{
    switch (value) {
        case 200:
            return "OK";
...
private:
    HttpCode _eHttpCode{};
    std::unordered_multimap<string, string> _mHeader{};
...
{
    output << "HTTP/1.1";
    auto code = resp._eHttpCode;
    output << ' ' << static_cast<size_t>(code);
    string result = GetCommentByValue((int)code);
    output << ' ' << result;
    output << '\n';
    for (const auto& [key, value] : resp._mHeader) {
        output << key << ':' << ' ' << value << '\n';
    }
    string content = resp._sContent;
    if (!content.empty()) {
...
void Test(CommentServer& srv, const HttpRequest& request, const ParsedResponse& expected) {
  stringstream ss;
  auto result = srv.ServeRequest(request/*, ss*/);
    ss << result;
  ParsedResponse resp;
  ss >> resp;
