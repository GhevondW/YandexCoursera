...
public:
    ...
    struct WriteAccess {
        WriteAccess(V& ref, std::mutex& m)
        :ref_to_value(ref),
        _lock(m){ /*_lock.lock();*/ }
        ~WriteAccess(){ _lock.unlock(); }
...
public:
    ...
    WriteAccess operator[](const K& key)
    {
        ...
        auto iter = _mData.find(index);
        std::mutex& m = iter->second.first;
        m.lock();
        return {iter->second.second[key], m};
    }
    ...
    MapType BuildOrdinaryMap() const
    {
        ...
        auto c = _mData.begin();
        while (c != end(_mData)) {
            ret.insert(begin(c->second.second), end(c->second.second));
            c++;
        }
        _DoUnlock();
        return ret;
...
private:
    ...
    void _Init()
    {
        for (size_t i = 0; i < _nBucketCount; ++i) {
            _mData[i];
        }
    }
    
    inline size_t _CalcIndex(const K& key) const
    {
        return _hasher(key) % _nBucketCount;
    }
    
    inline void _DoLock() const{
        auto c = _mData.begin();
        while (c != end(_mData)) {
            c->second.first.lock();
            c++;
    ...
    inline void _DoUnlock() const{
        auto c = _mData.begin();
        while (c != end(_mData)) {
            c->second.first.unlock();
            c++;
        }
    }
...
private:
    ...
    mutable std::map<size_t, std::pair<std::mutex, MapType>> _mData;
};
