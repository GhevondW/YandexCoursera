...
{
public:
    PolynomIndex(std::vector<T>& v, size_t pos) : coeffs_(v), pos_(pos) {}
...
template<typename T>
bool operator!=(const PolynomIndex<T>& index, const T& other)
{
...
template<typename T>
std::ostream& operator<<(std::ostream& out, const PolynomIndex<T>& index)
{
    if (index.pos_ >= index.coeffs_.size())
    {
        out << 0;
...
public:
    ...
    PolynomIndex<T> operator[](size_t pos)
    {
        return PolynomIndex<T>(coeffs_, pos);
    }
