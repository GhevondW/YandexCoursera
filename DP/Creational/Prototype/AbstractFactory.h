...
struct IAbstractFactory
{
    ...
    virtual std::shared_ptr<IButton> CreateButtonPtr(int w = 0, int h = 0, std::string text = "") = 0;
...
public:
    ...
    std::shared_ptr<IButton> CreateButtonPtr(int w, int h, std::string text) override
    {
        return std::make_shared<ButtonMac>(w, h, text);
    }
...
public:
    ...
    std::shared_ptr<IButton> CreateButtonPtr(int w, int h, std::string text) override
    {
        return std::make_shared<ButtonWindows>(w, h, text);
    }
